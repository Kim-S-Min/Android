...
public class MainActivity extends AppCompatActivity
    ...
    //  동적 권한 요청의 결과 콜백 메서드
    @Override
    public void onRequestPermissionsResult(int requestCode,     //  요청 코드 requestPermissions 에서 설정한 코드
                                           @NonNull String[] permissions,   //  요청 권한 문자열 배열
                                           @NonNull int[] grantResults) {   //  허가 결과에 대한 문자열
   ...
   //             callPhone();
    ...
    public void onClick(View v) {
        ...
        if (target == R.id.btnWeb) {
            ...
            //  사용자 정의 액선을 암시적으로 호출할 때
            //      정의 액션을 수행할 앱이 설치 되지 않을 가능성을 염두
            //  우리의 인텐트를 처리할 수 있는 앱이 있는지 확인
            if (intent.resolveActivity(getPackageManager()) != null) {  //  처리할 앱을 발견
                startActivity(intent);
            } else {    //  intnet처리할 앱이 없을 때
                Toast.makeText(MainActivity.this,
                        "해당 인텐트를 실행할 액티비티가 없습니다.",
                        Toast.LENGTH_LONG)
                        .show();
            }
